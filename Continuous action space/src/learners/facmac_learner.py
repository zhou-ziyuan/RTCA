...
class FACMACLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        for t in range(batch.max_seq_length):
            # print(batch)
            agent_target_outs, target_hidden_state_ = self.target_mac.select_actions(batch, t_ep=t, t_env=None, test_mode=True,
                                                               critic=self.target_critic, target_mac=True, hidden_states = self.target_hidden_state)
        ...
        for t in range(batch.max_seq_length):
            ...
            chosen_actions_x = self.mac.forward(batch, t=t, select_actions=True, hidden_states = self.hidden_state)
            agent_outs = chosen_actions_x["actions"].view(batch.batch_size,
                                                    self.n_agents,
                                                    self.n_actions)
            self.hidden_state = chosen_actions_x["hidden_state"]
