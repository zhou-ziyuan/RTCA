...
class FACMACDiscreteLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        for t in range(batch.max_seq_length):
            target_act_outs, target_hidden_state_ = self.target_mac.select_actions(batch, t_ep=t, t_env=t_env, test_mode=True, hidden_states = self.target_hidden_state)
        ...
        for t in range(batch.max_seq_length - 1):
            act_outs, hidden_state_ = self.mac.select_actions(batch, t_ep=t, t_env=t_env, test_mode=False, explore=False, hidden_states = self.hidden_state)
