...
def perturb_actions(xs, actions, n_actions, pix): # xs:[x, y] x为智能体id y为目标动作  img: 当前的动作
...
def predict_classes(xs, img, learner, state_batch, batch_size, n_action, agents_available_actions, pixels): # 扰动后的动作的MIX_Value值
    # print(1)
    imgs_perturbed = perturb_actions(xs, img.clone(), n_action, pixels) # 扰动后的动作
    xsbatch = len(xs)
    imgs_perturbed_ = imgs_perturbed.view(imgs_perturbed.size()[0],imgs_perturbed.size()[1]*imgs_perturbed.size()[2]).unsqueeze(1)


    # print(imgs_perturbed_.size())
    # print(state_batch.repeat(xsbatch, 1,1).size())
    predictions = learner.mixer(imgs_perturbed_,state_batch.repeat(xsbatch, 1,1))
    predictions = predictions.squeeze().data.cpu().numpy()
    # print(len(predictions))
    # aaa
    return predictions #if minimize else 1 - predictions

def attack_success(x, img, target_calss, learner, state_batch, batch_size, n_action, verbose=False, pixels = None): # 看攻击是否成功
    # Perturb the image with the given pixel(s) and get the prediction of the model
    attack_image = perturb_actions(x, img.clone(), n_action, pixels)
    attack_image_shape = attack_image.size()
    attack_image_ = attack_image.view(attack_image_shape[0],attack_image_shape[1]*attack_image_shape[2]).unsqueeze(1)
    # print(attack_image_.size())
    # print(state_batch.size())

    q_tot = learner.mixer(attack_image_, state_batch).data.cpu().numpy()
	# aaa
...
# learner: qmix 网络
# pixels：被攻击智能体的数量
def attack_q_de(img, label, learner, n_agent, n_action, state_batch, batch_size,agents_available_actions,
    ...
    predict_fn = lambda xs: predict_classes(
		xs, img, learner, state_batch, batch_size, n_action, agents_available_actions, pixels) # 要最小化的目标函数
    # x, img, target_calss, learner, action_values, state_batch, verbose=False
    callback_fn = lambda x, convergence: attack_success(
		x, img, target_calss, learner, state_batch, batch_size, n_action, verbose, pixels)
    # callback_fn = None
