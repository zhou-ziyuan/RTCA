...
def pgd(model, batch, actions, learner, mix_value_true, t,verbose=False,  env_id=""):
    ...
    for i in range(20):
        ...
        # print(X_adv.view(-1).unsqueeze(0)
        # )
        action_shape = actions.size()
        X_adv_ = X_adv.view(action_shape[0],action_shape[1]*action_shape[2]).unsqueeze(1)
        mix_value = learner.mixer(X_adv_, batch["state"][:, t:t + 1])#.data.cpu().numpy()
...
def attack_q_action(model, batch, actions, learner, mix_value_true, attack_config, t, loss_func=nn.CrossEntropyLoss()):
    ...
    adv_X = atk(model, batch, actions, learner, mix_value_true, t,verbose=False,  env_id="")
    return adv_X
