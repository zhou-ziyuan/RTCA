...
def perturb_actions_discrete(xs, actions): # xs:[x, y] x为智能体id y为目标动作  img: 当前的动作
...
def predict_classes_discrete(xs, img, learner, actions, state_batch, agents_available_actions): # 扰动后的动作的MIX_Value值
    imgs_perturbed = perturb_actions_discrete(xs, img.clone())
...
def attack_success_discrete(x, img, target_calss, learner, actions, state_batch, verbose=False): # 看攻击是否成功
    # Perturb the image with the given pixel(s) and get the prediction of the model
    attack_image = perturb_actions_discrete(x, img.clone())
	...
	# aaa
    # print(input.size())
    # print(torch.FloatTensor(state_batch).size())
...
# learner: qmix 网络
# pixels：被攻击智能体的数量
def attack_q_de_discrete(img, label, learner, actions,n_agent, n_action, state_batch,agents_available_actions,
    ...
    predict_fn = lambda xs: predict_classes_discrete(
    ...
    callback_fn = lambda x, convergence: attack_success_discrete(
    ...
    attack_image = perturb_actions_discrete(attack_result.x.astype(int), img.clone())
